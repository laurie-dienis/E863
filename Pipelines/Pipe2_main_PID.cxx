...
void Pipe2_main_PID(unsigned int runN) {
...
//  }
 ...
 if (!cutG) {
   std::cerr
       << "Error: Graphical cut 'cutG' not found in file 'cut_alpha_1.root'."
       << " Plotting PID without cut." << std::endl;
 }
 
 // List of file names
 const char* treeName = "DataTree"; // Tree name (common to all files)
 TString fileName = "RootFiles/Cal/15N/Cal" + runNumber + ".root";
...
// if (cutG) {
   ...
   newFile = TFile::Open("RootFiles/PID/15N/alpha_1_" + runNumber + ".root", "RECREATE");
