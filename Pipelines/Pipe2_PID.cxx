...
void Pipe2_PID() {
    ...
    const char *cutFileName = "Cuts/cut_alpha.root";
    ...
    if (!cutG) {
        std::cerr << "Error: Graphical cut 'cutG' not found in file 'cut_alpha.root'."
    ...
    // Create histograms
    TH2F *PID = new TH2F("PID", "PID", 3000, 0, 700000, 3000, 0, 10);
    ...
    if (cutG) {
        newFile = TFile::Open("RootFiles/PID/alpha.root", "RECREATE");
        if (!newFile || newFile->IsZombie()) {
            std::cerr << "Error creating new file: RootFiles/PID/alpha.root" << std::endl;
        ...
        PIDTree->Branch("Si_cal", &si_cal, "Si_cal/D");
    }
    ...
    for (const auto &fileName : fileNames) {
        ...
        tree->SetBranchAddress("Si_cal", &si_cal);
