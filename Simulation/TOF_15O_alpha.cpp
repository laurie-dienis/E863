...
//***************************************
int extraction_data_SRIM(const Char_t *file, Double_t EnergyPart[],
                         Double_t RangePart_in_Au[], Double_t SPelPart_in_Au[],
                         Double_t SPnuPart_in_Au[]) {
  std::ifstream in;
  in.open(file);
  Int_t nlines = 0;
  while (1) {
    in >> EnergyPart[nlines] >> SPelPart_in_Au[nlines] >>
        SPnuPart_in_Au[nlines] >>
        RangePart_in_Au[nlines]; // SP in keV/microns, E in keV, Range in
                                 // Angstrom
...
//***************************************
double loss_E_srim(double Ei, double distance, Double_t EnergyPart[],
                   Double_t SPel_Part[], Double_t SPnu_Part[], int N_energy) {
  // distance in mm
  double Etemp = Ei;
  double dpart = 0;
  double ddx = 1; // micron
  while (dpart <= distance * 1000) {
    Etemp =
        Etemp -
        0.001 * interpol(EnergyPart, SPel_Part, N_energy, Etemp * 1000.0) *
            ddx -
        0.001 * interpol(EnergyPart, SPnu_Part, N_energy, Etemp * 1000.0) * ddx;
    // std::cout<< interpol(EnergyPart, SPel_Part, N_energy, Etemp * 1000.0) <<"
    // interpol  "<<std::endl; std::cout<< Etemp * 1000.0  <<" interpol
    // "<<std::endl;
    dpart = dpart + ddx;
  }
  // std::cout <<" \n  "<<std::endl;
...
void TOF_15O_alpha() {
  ...
  extraction_data =
      extraction_data_SRIM(fileSRIM, EnergyRecoil, RangeRecoil_in_Si,
                           SPelRecoil_in_Si, SPnuRecoil_in_Si);
  ...
  TH2F *h1 = new TH2F("h1", "", 800, 1, 20, 800, 0,
                      109); // impulsion as a function of the energy
  ...
  for (Int_t n = 0; n < iter; n++) {
    ...
    double TOF_b = 177 / vb + unif(rng) -
                   4.4; // 28.6 is the distance for which we measure the TOF
    double TOF_B = 200 / vB;
    // cout << "\n TOF alpha is equal to " << TOF_b << "ns" ;

    double ELoss = loss_E_srim(Eb, 0.5, EnergyRecoil, SPelRecoil_in_Si,
                               SPnuRecoil_in_Si, N_energy);
