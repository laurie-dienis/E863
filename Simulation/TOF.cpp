...
void TOF() {
  ...
  target_thickness = 90; // Gold(Au) target, in micrometer
  ...
  // Random generator
  std::mt19937_64 rng;
  std::uniform_real_distribution<double> unif(1, 3);

  // definition of the 2D and 1D spectra
  TH2F *h1 = new TH2F("h1", "", 800, 0, 20, 8000, 0,
                      100); // impulsion as a function of the energy
  TH2F *h2 = new TH2F("h2", "", 800, 0, 20, 8000, 0,
                      100); // impulsion as a function of the energy
  ...
  for (Int_t n = 0; n < iter; n++) {
    ...
    double TOF_b = 28.6 / vb;
    // double TOF_b =
    //     28.6 / vb +
    //     unif(rng); // 28.6 is the distance for which we measure the TOF
    // std::normal_distribution<double> gauss_b(TOF_b, 5.89 / 2.35);
    // TOF_b = gauss_b(rng);
    ...
    double TOF_s = 28.6 / vs;
    // std::normal_distribution<double> gauss_s(TOF_s, 5.89 / 2.35);
    // TOF_s = gauss_s(rng);
    // double TOF_s =
    //     28.6 / vs +
    //     unif(rng); // 28.6 is the distance for which we measure the TOF
  ...
  {
    t1->DrawLatex(15, -4, "E (MeV)");
    t2->DrawLatex(-1.2, 30, "TOF (ns)");
  }
