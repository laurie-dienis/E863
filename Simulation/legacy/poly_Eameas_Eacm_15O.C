...
double scattered_theta() {
    TFile* file1 = new TFile("scattering/beamEntry/2.5umTi/theta_distri_entrance.root", "READ");
...
void poly_Eameas_Eacm_15O(){
    ...
    int aivalable_pressure_target[5]={92,183,274,350,400};
  ...
  /*  double exitWindow_width = 0.020; //mm
    ...
    double angle_min_Si = 9; //degree
    double angle_max_Si = 10;//degree
    ...
    //15O
    const Char_t *fileSRIM[2] ={Form("sp_srim/4He_4He_%iTorr.dat", aivalable_pressure_target[choice_pressure_target]),Form("sp_srim/15O_4He_%iTorr.dat", aivalable_pressure_target[choice_pressure_target])};
    std::cout<<Form("sp_srim/15O_4He_%iTorr.dat", aivalable_pressure_target[choice_pressure_target])<<std::endl;
    const Char_t *fileSRIM_window[2] ={"sp_srim/4He_Ti.dat","sp_srim/15O_Ti.dat"};
 ...
 for(int i=0;i<2;i++){
    ...
    TH2F *h1 = new TH2F("h1", "Emeas_Ecm", 300, 5, 15, 300, 1,5);
    h1->SetNameTitle(
    ...
    for(int b=0;b<Nbeam;b++){
        ...
        //std::cout<< Ebeam/15.001138<<std::endl;
        ...
        for(int j=0;j<Nreac;j++){
            ...
            if(eEjectil_lab>0 && thetaEjectil_lab<angle_max_Si && thetaEjectil_lab> angle_min_Si && compteur_reaction==0){
                compteur_reaction=1;
