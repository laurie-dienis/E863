...
void FitPolynomial(TH2F *hist) {
  ...
  TF2 *fitFunc =
      new TF2("fitFunc", "pol2", hist->GetXaxis()->GetXmin(),
              hist->GetXaxis()->GetXmax(), hist->GetYaxis()->GetXmin(),
...
void poly_Eameas_Eacm_15N() {
  ...
  srim->ReadTable(
      "light", TString::Format(
                   "/home/laurie/Analysis_e863/Inputs/SRIM/4He_4He_%dTorr.txt",
  ...
  srim->ReadTable(
      "beam", TString::Format(
                  "/home/laurie/Analysis_e863/Inputs/SRIM/15N_4He_%dTorr.txt",
                  aivalable_pressure_target[choice_pressure_target])
                  .Data());
  ...
  double thetaEjectil_lab, vEjectil_lab, eEjectil_lab, ElossEjectil,
      distEjectil, remainingDist, vEjectil_lab_meas, Ecm_reaction;
  double Emeas;
  int compteur = 0;
    ...
    while (positionBeam < (beam_target_width - dx)) {
      ...
      for (int j = 0; j < Nreac; j++) {
        ...
        if (eEjectil_lab > 0 && thetaEjectil_lab < angle_max_Si &&
            compteur_reaction == 0) {
          compteur_reaction = 1;
          eEjectil_lab = srim->Slow("light", eEjectil_lab, remainingDist,
  ...
  // Fit function
  TF1 *fitFunc = new TF1("fitFunc", "[0] + [1]*x + [2]*x*x", 0,
                         20); // Adjust the range [0, 20] as needed
  ...
  // function_EaMeas_EaCM->SetTitle(";E_{#alpha}^{lab} at Si detector (MeV);
  // E_{#alpha}^{cm} at elastic reaction (MeV)");
