...
void alphaDistri(){
	int Nion = 5000;
	TH1F* theta_distri = new TH1F("theta_distri_out","^{4}He@15MeV in target 8cm; #theta_{out} (deg);counts/0.01degree",500,0,5);
	double theta,x,y,z, i; double Pi = TMath::Pi();
	Double_t X[Nion], Y[Nion], Z[Nion];
	std::ifstream in;
	in.open("/Users/chloefougeres/Documents/experiment/experiments_proposed/4He_15O_4He_15O/analysis/scattering/4He/15MeV/range.dat");
	Int_t nlines = 0;
	while (1) {
		in >> i >> z >> x >> y;
	...
	g->SetTitle("^{4}He@15MeV in target 8cm ; X (mm); Y (mm)");
