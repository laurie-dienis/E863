...
void beamDistri(){
	int Nion = 5000;
	TH1F* theta_distri = new TH1F("theta_distri_entrance","Al 3#mum window; #theta_{beam} (deg);counts/0.1degree",500,0,5);
	double theta,x,y,z, i; double Pi = TMath::Pi();
	Double_t X[Nion], Y[Nion], Z[Nion];
	std::ifstream in;
	in.open("/Users/chloefougeres/Documents/experiment/experiments_proposed/4He_15O_4He_15O/analysis/scattering/beamEntry/4umAl/range.dat");
	Int_t nlines = 0;
	while (1) {
		in >> i >> z >> x >> y;
	...
	g->SetTitle("Al 3#mum window;X (mm); Y (mm))");
