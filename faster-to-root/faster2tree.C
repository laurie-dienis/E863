...
#define DATAFILENAME "data/E863_run_019_0001.fast"
#define ROOTFILENAME "root/run_019.root"
...
int main (int argc, char** argv) {
 ...
 // Int_t                  leaf_q1;
  Double_t        leaf_Si_1, leaf_TAC_1, leaf_Si_2, leaf_TAC_2, leaf_faster_time_1, leaf_faster_time_2, clock_Si_1, clock_tac_1, clock_Si_2, clock_tac_2;
  //  root tree
  TTree*                 tree;
  char                   tree_title [256];
  //  root file
   ...
   if (alias == CRRC4_SPECTRO_TYPE_ALIAS) {
	...
	if (label == 2) {                                                   //  
	  ...
	  clock_Si_1 = faster_data_clock_ns(data);
 	  ...
 	  if ((data = faster_file_reader_next (reader)) != NULL) {                    //  read each data
          label = faster_data_label(data);
